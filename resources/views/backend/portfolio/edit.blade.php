      ...
      <form method="post" action="{{route('portfolio.update',$project->id)}}">
        ...
        <div class="form-group">
          ...
          <select name="category" class="form-control">
            <option value="1" {{(($project->category=='1') ? 'selected' : '')}}>Sugar Mill Project</option>
            <option value="2" {{(($project->category=='2') ? 'selected' : '')}}>Design</option>
            <option value="3" {{(($project->category=='3') ? 'selected' : '')}}>Web</option>
          </select>
          @error('category')
          <span class="text-danger">{{$message}}</span>
          @enderror
